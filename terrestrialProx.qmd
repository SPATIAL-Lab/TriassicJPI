...
* The posterior MATs skew a little bit cooler than you would get if you used these data alone, but are not too dissimilar on average.
* Carnian site 3 (Chanares) stands out as having reconstructed MAT substantially lower than the Al/Si would imply, alone.

## CIA

Same thing for CIA, which is available at 4 Carnian and 2 Anisian sites.
...
* In this case, the posterior MATs tend to be a little higher than you'd expect just based on the proxy data (top/right), with the opposite true for one  Anisian site (bottom/left).
...
Now paleosol weathering index, only available at two Carnian sites. I've added the calibration curve using the median curve fit parameters in the posterior for reference.
...
* The JPI posterior is pretty much spot on after accounting for seasonality. The closed symbols are MAT, the open ones are summer temperature.
* Note that this same site has a bunch of CIA data shown above, and the posterior MAT is at the warm side of the range that you'd predict based on the CIA data alone. Incorporating the assumption that the clumped data represent warm-season temperatures allows lower MAT at this site, making the CIA results more congruent.
...
Univariate interpretations of the terrestrial proxy data at all sites *except* Utah suggest MATs no higher than 20$^\circ$. Incorporation of some (probably valid) assumptions like warm-season precipitation of terrestrial carbonates has increased the univariate MAT estimates somewhat compared with our original analyses. The estimates from the full JPI, however, were already being influenced by the marine proxy data and the prior constraints on the meridional MAT gradient. As a result, in the initial analysis they were, if anything, warmer than one would have interpreted using the individual proxies in isolation. The reconstruction has become more consistent overall through the changes that have been made, but the high-latitude JPI temperatures have not changed too much and seem to be a good reflection of what the proxies are telling us.
